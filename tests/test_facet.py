...
class TestFacet:
    ...
    # Периметр единичного квадрата = 8
    def test_perimeter01(self):
        f = Facet([R3(1.0, 1.0, 0.0), R3(1.0, -1.0, 0.0),
                   R3(-1.0, -1.0, 0.0), R3(-1.0, 1.0, 0.0)])
        assert f.perimeter() == approx(8.0)

    # Периметр заданного треугольника = 2+sqrt(2)
    def test_perimeter02(self):
        f = Facet([R3(1.0, 0.0, 0.0), R3(0.0, 1.0, 0.0),
                   R3(0.0, 0.0, 0.0)])
        assert f.perimeter() == approx(2+sqrt(2))
