...
from r3 import R3
from polyedr import Segment, Edge, Facet
...
class TestEdge:
    ...
    # Затенённое ребро не видно
    def test_is_seen_01(self):
        s = Edge(R3(0.0, 0.0, 0.0), R3(1.0, 1.0, 0.0))
        f = Facet([R3(0.0, 0.0, 1.0), R3(2.0, 0.0, 1.0),
                   R3(2.0, 2.0, 1.0), R3(0.0, 2.0, 1.0)])
        s.shadow(f)
        assert s.is_seen() == 0

    # Освещённое ребро видно
    def test_is_seen_02(self):
        s = Edge(R3(3.0, 3.0, 0.0), R3(4.0, 4.0, 0.0))
        f = Facet([R3(0.0, 0.0, 1.0), R3(2.0, 0.0, 1.0),
                   R3(2.0, 2.0, 1.0), R3(0.0, 2.0, 1.0)])
        s.shadow(f)
        assert s.is_seen() == 1

    # Частично освещённое ребро видно частично
    def test_is_seen_03(self):
        s = Edge(R3(0.0, 0.0, 0.0), R3(4.0, 4.0, 0.0))
        f = Facet([R3(0.0, 0.0, 1.0), R3(2.0, 0.0, 1.0),
                   R3(2.0, 2.0, 1.0), R3(0.0, 2.0, 1.0)])
        s.shadow(f)
        assert s.is_seen() == 1j
